...
1. **Trust** — every number on screen is derived, capped, evidenced, and reproducible. We never fabricate. If we have nothing, we say so and refund.
   - **Accuracy moat / LLM-noise elimination (Stefano's #1, 2026-06-13).** Dogfooding proved the deterministic tier is 11/11 truthful while the LLM ships false claims of *absence* in domains instruments already own. Full plan: `docs/LLM_NOISE_ELIMINATION_PLAN.md`; evidence ledger: `docs/DETECTION_SOURCE_ACCURACY.md`.
     ...
     - **P1 — DOM-verification gate DONE (2026-06-14):** `pipeline/dom-verification.ts` refutes LLM absence-claims against a per-page rendered-DOM snapshot (`DomFacts`: landmarks, headings, form-label coverage, links, lang, viewport) captured in the browser pass (`wcag-checker.captureDomFacts`) and threaded through the serialized `parallel-site-checks` step as a plain object. Gate 2e in `process-audit.ts`; never drops without positive contradicting evidence; per-page (a /contact claim is checked against /contact's DOM). 11 tests. **Evidence-binding DONE (2026-06-14):** `pipeline/evidence-binding.ts` — an LLM finding with no verbatim quote AND no DOM selector is ungrounded → confidence dropped into the undetermined band (gate 2f), so the severity≤evidence invariant caps it at LOW/off-the-cap. Enforces the /methodology "must cite what it saw" promise structurally. **Latent bug fixed same change:** the quality-gate in-memory finding shape omitted `confidence_score`/`evidence`/`viewport`, so gate 4b (severity≤evidence) had been reading undefined confidence and never clamped undetermined findings in prod — now loaded at the single fetch (line ~3771). P1 layer complete (domain + DOM + grounding).
     - **P2 — DONE (2026-06-14):** `eval/truth-set.ts` (labeled, seeded from fixpath run 1) + `eval/precision.ts` (per-source FP-elimination + false-drop over the live gates) + deploy-gate test (`__tests__/precision-harness.test.ts`) asserting 100% FP elimination and 0% false-drop.
     - **P3 — feed-the-model DONE (2026-06-14):** `formatDomFactsForPrompt` injects the verified DOM structure ("do NOT report these as missing") into the analyzer's `fullContext` (build-site-context), preventing absence-hallucinations at generation time; `DomFacts` snapshot now persisted to `report.raw_json.domFacts` (JSONB, no migration). **P3 dismissal telemetry DONE (2026-06-14):** `eval/dismissal-telemetry.ts` — classifies each dismissal reason (inaccurate / wont_fix / not_relevant / duplicate / other) so only *inaccurate* dismissals count as noise, then aggregates per detection_source and per evidence tier (Verified vs AI-assessed) as a live FIELD-precision signal complementing the P2 lab metric. No migration (reads dismissed/reason/detection_source off the finding row; workspace scoping resolves audit ids via the audits table since audit_findings has no workspace_id). The moat is feature-complete (P0→P3).
