...
const PILLAR_NAMES = [
  ...
  'Future Readiness', 'SEO Structure & Rules', 'Brand Consistency',
]
...
export async function GET(
  ...
  try {
    ...
    doc.moveDown(0.2)
    doc.fontSize(10).font('Helvetica').fillColor(T.muted)
      .text(`${activeFindings.length} findings across ${PILLAR_NAMES.length} modules and 24 categories`)
    ...
    if (categoryScores.length > 0) {
      ...
      doc.fontSize(9).font('Helvetica').fillColor(T.muted)
        .text('Scores across 6 modules and 24 categories, rated 0 to 100.', M, undefined, { width: CW })
      doc.moveDown(0.8)
    ...
    bodyText(
      ...
      'The audit evaluates up to 6 modules and 24 categories covering UX, accessibility, SEO, brand ' +
